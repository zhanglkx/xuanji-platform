...
const mySqrt = function (x) {
    ...
    let left = 0, right = x, middle = Math.floor((left + right) / 2);
    let condition = (Math.pow(middle + 1, 2) > x) && (Math.pow(middle, 2) <= x);
    while (!condition) {
        middle = Math.floor((left + right) / 2);
        if ((middle * middle === x) || ((Math.pow(middle + 1, 2) > x) && (Math.pow(middle, 2) <= x))) {
            return middle;
